<template>
  <div class="datepicker-container" v-click-outside="test">
    <input type="text" v-model="formatDate" placeholder="选择时间" class="el-input__inner" readonly>
    <!-- <el-input></el-input> -->
    <!-- <el-input v-model="value" placeholder="请输入内容"></el-input> -->
    <div v-if="isVisible" class="datepicker-con el-picker-panel el-date-picker el-popper" :class="isTop">
      <div class="header">
        <i class="icon el-icon-d-arrow-left" @click="switchYear('prev')"></i>
        <i class="icon el-icon-arrow-left" @click="switchMonth('prev')"></i>
        <h5 class="title">
          <span v-text="`${cloneDate.year}年`"></span>
          <span v-text="`${cloneDate.month + 1}月`"></span>
        </h5>
        <i class="icon el-icon-arrow-right" @click="switchMonth('next')"></i>
        <i class="icon el-icon-d-arrow-right" @click="switchYear('next')"></i>
      </div>
      <div class="body">
        ...
        <ul class="day-con day-list">
          ...
          >
            <span
              ...
              :class="[{'check':isSelect(day)},{'today': isToday(day)}]"
            >
            </span>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    isDayDisabled(date) {
      let {year, month} = util.getYearMonthDay(util.getDate(this.cloneDate.year, this.cloneDate.month, 1))
